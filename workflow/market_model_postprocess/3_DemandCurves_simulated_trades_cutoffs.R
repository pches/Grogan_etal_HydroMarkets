####################################################################################
####################################################################################
## Script Name: 3_DemandCurves_simulated_trades_cutoffs.R##
...
## by the water market and the amount of water rights can be traded under the new cumulative curve
## to calculate the welfare gains from water rights trading. The outputs of this script
## are a list of excel files that will be used in the 4_WelfareCalculation.R file to calculate the 
## welfare gains by WMAs. This scripts generates results under two scenarios: without SGMA and with SGMA.
##
##
##
...
## spatial WWR water trade regions; cumulative water rights curves with WBM inputs
##
...
stateRegVol <- mapply(convertWMAVolToTrdRegions, st=stateNames, stPrices=statePricesByReg, stateHydroReg=stateRegSpData, MoreArgs=list(surRights=wmasRightsTotsSur, grdRights=wmasRightsTotsGrd, wmaLayer=wmasData, areaProj=projForAreaCalc, whichYear="cutoff", cutOffYrs=datesWithIDs), SIMPLIFY=F)
stateRegVol <- mapply(convertWMAVolToTrdRegions, st=stateNames, stPrices=statePricesByReg, stateHydroReg=stateRegSpData, MoreArgs=list(surRights=wmasRightsTotsSur, grdRights=wmasRightsTotsGrd, wmaLayer=wmasData, areaProj=projForAreaCalc, whichYear="cutoff", cutOffYrs=datesWithIDsSMGA), SIMPLIFY=F)
...
mapply(writeEconModelResults, modResults=stEconModels, st=stAbbr, wDir=paste0(wwrDir, "waterRunsOutCutoffs/"))
mapply(writeEconModelResults, modResults=stEconModels, st=stAbbr, wDir=paste0(wwrDir, "waterRunsOutCutoffs_SMGA/"))


modWaterRights <- mapply(applyTrdRegionsToWMAs, st=stateNames, modeledEconVals=stEconModels, spReg=stateRegVol, wmaInReg=stateRegWMAPer,
                          MoreArgs=list(surRights=wmasRightsTotsSur, grdRights=wmasRightsTotsGrd, wmaLayer=wmasData, areaProj=projForAreaCalc, 
                                        ...
                                        typeApplicaiton="earlyPrioDates", surYrCutTab=wmaCutOffDates))
